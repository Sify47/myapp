...
import 'package:cloud_firestore/cloud_firestore.dart'
    hide Order; // Needed for Timestamp
import 'package:intl/intl.dart'; // For number formatting
import '../../../models/order.dart';
import '../../../services/order_service.dart';

class ReportsBrandPage extends StatefulWidget {
  const ReportsBrandPage({super.key});

  @override
  State<ReportsBrandPage> createState() => _ReportsPageState();
}

class _ReportsPageState extends State<ReportsBrandPage> {

  @override
  Widget build(BuildContext context) {
    final orderService = Provider.of<OrderService>(context);
    final numberFormat = NumberFormat.currency(locale: 'ar_SA', symbol: 'ج.م');
    final auth = Provider.of<AuthModel>(context, listen: false);

    return Scaffold(
      appBar: AppBar(title: Text(auth.brandId as String), centerTitle: true),
      body: StreamBuilder<List<Order>>(
        // Fetch all orders for reporting (consider pagination/filtering for large datasets)
        stream: FirebaseFirestore.instance
            .collection('orders')
            .where('brandIds', arrayContains: auth.brandId)
        ...
        builder: (context, snapshot) {
          ...
          final totalDiscountGiven = orders.fold(
            0.0,
            ...
            child: Column(
              ...
              children: [
                ...
                _buildReportCard(
                  ...
                  numberFormat.format(totalDiscountGiven),
                  Icons.discount,
                  Color(0xFF3366FF),
                ),
